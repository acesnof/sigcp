...
def build():
    ...
    body(doc, "A Gestão de Férias concentra decisões, planeamento, calendário e relatórios. Administradores e SNR podem decidir todos os pedidos pendentes, incluindo períodos passados.")
    ...
    body(doc, "Na aba Pessoal e Direitos são apresentadas, por defeito, apenas as pessoas que permanecem na missão, ordenadas por posto e pela antiguidade mais antiga. O botão Mostrar todos inclui todo o pessoal da base de dados; quem já saiu surge depois, ordenado por posto e nome. O filtro Incluir Passadas da aba Pedidos mantém-se independente.")
    ...
    body(doc, "Administradores e SNR podem aprovar, devolver ou rejeitar todos os pedidos conforme o estado, mesmo quando o período já passou. Apenas o próprio titular pode usar Corrigir num pedido pendente ou devolvido. Os Administradores dispõem do botão Apagar em qualquer fase; esta eliminação é permanente e deve ser usada apenas para corrigir registos indevidos.")
